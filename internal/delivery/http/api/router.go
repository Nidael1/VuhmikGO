...
func RegisterAPIRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/v1/auth/accept-terms", JWTMiddleware(HandleAcceptTerms))
	...
	mux.HandleFunc("/api/v1/admin/system/recalculate", JWTMiddleware(AdminMiddleware(HandleAdminSystemRecalculate)))
	mux.HandleFunc("/api/v1/admin/system", JWTMiddleware(AdminMiddleware(HandleAdminSystem)))
	mux.HandleFunc("/api/v1/admin/health/", JWTMiddleware(AdminMiddleware(adminHealthDispatcher)))
	...
	mux.HandleFunc("/api/v1/admin/metrics/accounts/", JWTMiddleware(AdminMiddleware(HandleAdminMetricsAccountDetail)))
	mux.HandleFunc("/api/v1/admin/activity", JWTMiddleware(AdminMiddleware(HandleAdminActivity)))
