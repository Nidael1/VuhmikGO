...
async function save() {
  ...
  try {
    const vPayload = {
      ...
      tiene_receta: showRx.value,
    }

    // 1. Crear consulta
    const con = await consultationRepository.create(pid, vPayload)

    // 2. Crear nota vinculada si el médico la escribió (opcional)
    if (nota.value.trim()) {
    ...
    // 3. Crear y emitir receta si aplica
    let rxId = ''
  ...
  <AppLayout>
    <div class="page">
      ...
      <div class="card">
        ...
        <!-- Nota de consulta (opcional) -->
        <div class="section">
          <h3 class="section-title">Nota de consulta <span class="optional">(opcional)</span></h3>
          <textarea
            v-model="nota"
            rows="5"
            class="input-field"
            placeholder="Observaciones de esta consulta..."
            maxlength="2000"
          />
          <span class="char-count">{{ nota.length }} / 2000</span>
        </div>

        <!-- Receta electrónica (opcional) -->
        <div class="section">
          <div class="section-header">
            <h3 class="section-title">Receta electrónica <span class="optional">(opcional)</span></h3>
